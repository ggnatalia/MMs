...
def main(args): ######################################################### CONVERTIR DB EN SQL PARA AGILIZAR SU CONSTRUCCIÓN Y LAS QUERIES?
    path = os.getcwd()
    ...
    os.chdir(mockPath)
    # List of sequences. 
    #newSeqs = Sequence.set_sequences(fastaFile = '{}/{}'.format(projectPath, args.align), refTax = refTax, degap = False, cleanHeader = False, splitChar = '\t', conservative = False)
    ...
    command = ['mothur','#dist.seqs(fasta = {}/{})'.format(projectPath, args.align)] # Saline.dist
    runCommand(command)
    #shutil.move('{}/{}.dist'.format(projectPath, projectPrefix), '{}/{}.dist'.format(projectPath, projectPrefix))
    ...
    # Remove total 0 sequences ffrom distance file
    ...
    plt.title('{} shannonIndex rank'.format(mockPrefix))
    plt.xlabel('rank')
