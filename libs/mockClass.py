...
class Mock():
    ...
    @classmethod
    def init_from_previous_mock(cls, Enviro, prefix, shannon, alignment = [0, 50000, '16S'], reads = 20000, errormodel = 'perfect', figsize = (20,20), cpus = 12, inSilicoparams = (150,200)): # Esta es para corregir antiguas, pero se puede quitar
    ...
    def init_from_enviro(cls, enviro, prefix, Nsamples, shannon, alignment = [0, 50000, '16S'], alpha = 0.9,  smallest_coef = 0.1, largest_coef = 0.9, reads = 20000, pstr0 = 0.2, size = 1, errormodel = 'perfect', figsize = (20,20), cpus = 12, inSilicoparams = (150,200)):
        ...
        if Nsamples == 1:
            abunTable = np.array(list(map(round, cls.make_global_samples_distribution(shannon = shannon, nASVs = len(enviro.Seqs), reads = reads).T )))#lognormal distribution
    ...
    @staticmethod
    def make_global_samples_distribution(shannon, nASVs, reads = 20000):
        """Calculate the abundances distribution based on a shannon Index (H). Formula approach (Edden 1971)"""  
        if (-2*(shannon - np.log(nASVs))) < 0:
            write_logfile('warning', 'ABUNDANCE ESTIMATION SHANNON', -2*(shannon - np.log(nASVs)))
            exit(-5)
        else:
            sd = math.sqrt(-2*(shannon - np.log(nASVs)))
            mu = 0
    ...
    @staticmethod
    def ZINBD(nASVs, nSamples, CorrMatrix, shannon, reads = 20000, pstr0 = 0.2, size = 1):
        ...
        abundanceTable = qzinegbin(probNormal, size = size, munb = Mock.make_global_samples_distribution(shannon, nASVs, reads), pstr0 = pstr0, nVariables = nASVs)
